...
do
	...
	function internalSettings:Initialize()
		...
		if identifyexecutor then
			local hostData = internalUtils:Request("https://rblxexploits.com/codexiosUI.json");
...
do
	...
	local function createUI(directory: Instance): ScreenGui
		ui = instanceUtils:Create("ScreenGui", {
			...
			instanceUtils:Create("Frame", {
				...
				textButton({ -- SPDM Team | Buy Premium Button
					...
					MouseButton1Click = function()
						if setclipboard then
							setclipboard("https://codex-collective.mysellix.io/");
						end
						cloneref(game:GetService("StarterGui")):SetCore("SendNotification", {
							Title = "Codex iOS",
		...
		task.spawn(function()
			...
			if not (cloneref(game:GetService("RunService")):IsStudio() or internalUtils:AreVersionsAlike(currentVersion, internalSettings.data.iosVersion)) then
...
do
	...
	local function createNavbar(gui: ScreenGui): Instance
		...
		local isDragging = false
		bar.floatingIcon.MouseMoved:Connect(function()
			isDragging = true
		end)
		
		bar.floatingIcon.MouseButton1Up:Connect(function()
			wait(0.1)
			isDragging = false
		end)

		bar.floatingIcon.MouseButton1Click:Connect(function()
