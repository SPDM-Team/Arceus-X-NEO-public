...
do
	...
	function internalSettings:Initialize()
		...
		if identifyexecutor then
			local hostData = internalUtils:Request("https://rblxexploits.com/codexUI.json");
...
do
	...
	local function createUI(directory: Instance): ScreenGui
		...
		task.spawn(function()
			...
			if not (cloneref(game:GetService("RunService")):IsStudio() or internalUtils:AreVersionsAlike(currentVersion, internalSettings.data.androidVersion)) then
